...
class ClienteModel {
    ...
    public function getClientesByCedulasYCarga($cedulas, $cargaId) {
        ...
        // Esquema emermedica: clientes.base_id (no existe carga_excel_id en la tabla)
        $placeholders = implode(',', array_fill(0, count($cedulas), '?'));
        $sql = "SELECT " . $this->selectClienteCompatFields() . "
                FROM clientes c
                WHERE c.cedula IN ($placeholders) AND c.base_id = ?";
        
        $params = array_merge(array_map('strval', $cedulas), [(int)$cargaId]);
        $stmt = $this->pdo->prepare($sql);
        $stmt->execute($params);
        ...
        foreach ($resultados as $cliente) {
            $indexados[(string)($cliente['cedula'] ?? '')] = $cliente;
        }
    ...
    public function crearClientesBulk($clientes) {
        ...
        /**
         * INSERT múltiple al esquema real `clientes` (base_id, tel1..tel10).
         * El código legacy usaba columnas inexistentes (carga_excel_id, telefono, …) y fallaba en silencio
         * en cargas optimizadas (30k+ filas), dejando 0 clientes insertados.
         */
        $idsCreados = [];
        $chunkSize = 150;

        $filaVals = function (array $c): array {
            $baseId = (int)($c['carga_excel_id'] ?? $c['base_id'] ?? 0);
            ...
            $tel = [];
            $tel[0] = (string)($c['telefono'] ?? $c['tel1'] ?? '');
            $tel[1] = (string)($c['celular2'] ?? $c['tel2'] ?? $c['telefono2'] ?? '');
            for ($i = 2; $i < 10; $i++) {
                $n = $i + 1;
                $tel[$i] = (string)($c['tel' . $n] ?? $c['cel' . $n] ?? '');
            }

            return [$baseId, $cedula, $nombre, $email, $ciudad, $tel[0], $tel[1], $tel[2], $tel[3], $tel[4], $tel[5], $tel[6], $tel[7], $tel[8], $tel[9]];
        };

        $chunks = array_chunk($clientes, $chunkSize);
        foreach ($chunks as $chunk) {
            $sql = "INSERT INTO clientes (base_id, cedula, nombre, email, ciudad, tel1, tel2, tel3, tel4, tel5, tel6, tel7, tel8, tel9, tel10, estado) VALUES ";
            $parts = [];
            $params = [];
            foreach ($chunk as $c) {
                $parts[] = '(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, \'activo\')';
                $params = array_merge($params, $filaVals($c));
            }
            $sql .= implode(', ', $parts);
            ...
            $primerId = (int)$this->pdo->lastInsertId();
            $n = count($chunk);
            for ($i = 0; $i < $n; $i++) {
